...
namespace WEB2022Apr_P01_T3.Controllers
{
    public class ProductManagerController : Controller
    {
        ...
        //to popuplate list for obsolete and new choices
        private List<SelectListItem> GetCondition(ProductManager product)
        {
            ...
            //if date more than 365 days then add obsolete to list otherwise only add new
            if ((DateTime.Now - product.productEffectiveDate).TotalDays > 365)
            {
                conditions.Add(new SelectListItem
                {
                    Value = "Obsolete",
        ...
        {   
            ...
            if (product.photoUpload != null && product.photoUpload.Length > 0)
            {
                try
                {
                    ...
                    string uploadedFile = product.productName + fileExt;
